...
namespace exafmm
{
  ...
  void write_snapshot(int snapshot_num, struct sys s)
  {
    ...
    space_id = H5Screate_simple(1, dims, NULL);
    for(unsigned int b = 0; b < s.n; b++)
      data[b] = s.part[ids[b]].jerk[0];
    dset_id = H5Dcreate(file_id, "Jerkx", H5T_NATIVE_DOUBLE, space_id, H5P_DEFAULT);
    status = H5Dwrite(dset_id, H5T_NATIVE_DOUBLE, memspace, space_id, H5P_DEFAULT, data);
    status = H5Sclose(space_id);
    status = H5Sclose(memspace);
    status = H5Dclose(dset_id);
    ...
    space_id = H5Screate_simple(1, dims, NULL);
    for(unsigned int b = 0; b < s.n; b++)
      data[b] = s.part[ids[b]].jerk[1];
    dset_id = H5Dcreate(file_id, "Jerky", H5T_NATIVE_DOUBLE, space_id, H5P_DEFAULT);
    status = H5Dwrite(dset_id, H5T_NATIVE_DOUBLE, memspace, space_id, H5P_DEFAULT, data);
    status = H5Sclose(space_id);
    status = H5Sclose(memspace);
    status = H5Dclose(dset_id);
    ...
    space_id = H5Screate_simple(1, dims, NULL);
    for(unsigned int b = 0; b < s.n; b++)
      data[b] = s.part[ids[b]].jerk[2];
    dset_id = H5Dcreate(file_id, "Jerkz", H5T_NATIVE_DOUBLE, space_id, H5P_DEFAULT);
    status = H5Dwrite(dset_id, H5T_NATIVE_DOUBLE, memspace, space_id, H5P_DEFAULT, data);
    status = H5Sclose(space_id);
    status = H5Sclose(memspace);
    status = H5Dclose(dset_id);
