		...
		double dt, bool update_timestep)
{
  ...
  real_t *jerk  = new real_t[3 * bodies.size()];
  ...
  for(size_t b = 0; b < bodies.size(); b++)
    {
      ...
      jerk [3 * i + 0] = bodies[b].J[0];
      jerk [3 * i + 1] = bodies[b].J[1];
      jerk [3 * i + 2] = bodies[b].J[2];
  ...
  for(size_t i = 0; i < sinks.n; i++)
    {
      ...
      sinks.part[i].jerk[0] = jerk[i * 3 + 0];
      sinks.part[i].jerk[1] = jerk[i * 3 + 1];
      sinks.part[i].jerk[2] = jerk[i * 3 + 2];
#endif
    }
