...
/*
  ...
  horizontalPass(newcells, newcells, 1);
...
struct sys join(struct sys sinks, struct sys sources)
{
  ...
  if(sinks.n == 0) return sources;
  if(sources.n == 0) return sinks;  
                          ...
                          double stime, double etime, double dt, int calc_timestep)
{
  ...
  if(slow.n>0) 
    if(slow.n>0) kick_naive(clevel, slow, fast, sys2, dt/2, update_timestep);

  //kick(clevel, slow, join(sys1,sys2), dt/2, update_timestep, sinks_are_fast);
}

void evolve_split_hold_dkd(int clevel,struct sys s, double stime, double etime, double dt, bool calc_timestep)
{
  struct sys slow=zerosys,fast=zerosys;
...
double system_kinetic_energy(struct sys s) {
 unsigned int i;
 long double e=0.;
 for(i=0;i<s.n;i++) e+= s.part[i].mass/G*(
...
double system_potential_energy(struct sys s)
{
 unsigned int i;
 long double e=0.;
 for(i=0;i<s.n;i++) e += s.part[i].mass/G*s.part[i].pot/2;
 return (double) e;
}

void system_center_of_mass(struct sys s, long double *cmpos, long double *cmvel)
{
  long double mass=0.,pos[3]={0.,0.,0.},vel[3]={0.,0.,0.};
