	...
	void initKernel() {
		...
		for (int i = 0; i <= P; i++) {
			for (int j = 0; j <= P; j++) {
				...
				if(j<= i)
					combinator_coef[i][j] = factorial_table[i]*inverse_factorial_table[j]*inverse_factorial_table[i-j];
			}
	...
	void real_2_complex(real_t*real_arr, complex_t*complex_arr) {
		for (int n = 0; n <= P; n++) {
			...
			for (int m = 1; m <= n; m++) {
				...
				complex_arr[index_start+m] = complex_t(real_arr[index_start+m],
							real_arr[index_start - m]);
				oddevenfac *= -1;
			}
		}
	}
      ...
      for(int n=1; n<=Order; n++) {
		...
		int m, m2, indexstart = n2-n, indexnew = n2+n;
	  	...
	  	if(n > 1){
        	for(m=0; m<n-1; m++){
				m2  = m*m;
          		Tnm[indexnew+m] = invr2 * (2*n-1.) * dX[2] * Tnm[indexstart+m] - invr2 * ((n-1)*(n-1)*(n>1) - m*m) * Tnm[n2-3*n+2+m];
        	}
      	}
      ...
      for(int n=1; n<=Order; n++) {
		...
		int m, m2, indexstart = n2-n, indexnew = n2+n;
	  	...
	  	if(n > 1){
        	for(m=0; m<n-1; m++){
				m2  = m*m;
          		Gnm[indexnew+m] = (2*n-1.)/(n2-m2) * dX[2] * Gnm[indexstart+m] - r2/(n2-m2) * Gnm[n2-3*n+2+m];
        	}
      	}
	...
	void P2P(Cell*Ci, Cell*Cj) {
		...
		int  nj = Cj->NBODY;
			...
			for (int i = 0; i < nii; i = i+NSIMD) {
				...
				ax = 0, ay = 0, az = 0, pot = 0, timestep=HUGE;
	...
	void M2M(Cell*Ci) {
		...
		if (1) {
			if (Ci->NCHILD > 0) {
				...
				for (Cell*Cj = Ci->CHILD; Cj != Ci->CHILD+Ci->NCHILD; Cj++) {
					...
					if(Cj->has_sink) Ci->has_sink=true;
		...
		for (Cell*Cj = Ci->CHILD; Cj != Ci->CHILD+Ci->NCHILD; Cj++) {
			for (int d = 0; d < 3; d++) {
				dX[d] = Cj->X[d] - Ci->X[d];
				if(isnan(dX[d]))
					ENDRUN("cell position error %g %g \n", Ci->X[d], 
					Cj->X[d]);
			}
		...
		for (int indice = 0; indice < NTERM; indice++) {
			if(isnan(Ci->M[indice])) {
				ENDRUN("M2M error M[indice] \n", indice);
			}
		}
	}
